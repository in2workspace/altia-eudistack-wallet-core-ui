...
- **EUD-104 — "Mis dispositivos" showing wrong or missing active sessions**: `LoginPage` now tells the backend which passkey verified the current session instead of leaving it permanently unattributed. `verifyPasskey()` calls the new `PasskeyApiService.confirmSession()` right after a successful WebAuthn assertion on a returning device (previously that verification was purely local and the backend never learned about it), and `createPasskeyForDevice()` now sends the current refresh token when registering a new passkey on `POST /api/v1/auth/passkeys`. Best-effort: a failed confirmation never blocks login, it just leaves that one session out of the devices list, same as before this fix.
