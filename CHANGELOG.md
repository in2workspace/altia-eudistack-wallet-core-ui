...
- **Device re-registration demanded again on a different-day login, non-deterministically — even on a device already linked to the account**: `resolvePasskeySetupStep()` forced `needsPasskeySetup = true` on the very first `listPasskeys()` failure, turning any transient network/5xx blip into a forced device re-registration indistinguishable from a genuinely unlinked device — now retries once before falling back. Neither login path told the backend which session its passkey belonged to: the "device already recognized" path never called the (already existing but unused) `POST /passkeys/{id}/confirm-session`, and `createPasskeyForDevice()` never sent its `refreshToken` on `POST /passkeys` either, so most sessions stayed unattributed and one device's refresh-token race could still knock out others (see the `eudistack-core-wallet-ebw` changelog for the matching backend fix). Attribution runs on both entry points now, including the different-day resume path (`ionViewWillEnter` with a stored refresh token), which never lists passkeys — there this device's server passkey is resolved from its local credential id before calling `confirm-session`. `createPasskeyForDevice()` also left the local `credential_id` pointing at a passkey the server never learned about whenever the server-side `POST` failed after the WebAuthn ceremony had already succeeded, and every attempt minted a brand-new random WebAuthn `user.id` — a retry created another orphaned resident credential instead of replacing the failed one. Local state now rolls back on a failed registration, and the WebAuthn user handle (`PasskeyStoreService`) is stable and persisted **per account** (`webauthn_user_id:<email>`), so a retry by the same account reuses its handle while a different account on the same browser profile gets its own and never overwrites the first account's resident credential.
...
- **Menu pages redesigned, with Settings as a single accordion screen** — presentation only; no change to what any of these settings do.
  - **Header menu**: the Language entry becomes **Settings** (gear icon) pointing at `/tabs/settings`; the menu is now Activity, Connected devices, Settings, About and Logout.
  - **Settings**: rebuilt as three collapsible sections following the About page pattern — **Language** (native language cards plus the runtime UI-translation controls), **Theme** (light/dark switch) and **Selected camera** (device picker). Both dropdowns now open from anywhere in their row, not just the caret.
  - **About, Activity and Connected devices**: adapted to the new mock — collapsible rows in About, a redesigned activity list with its filter and empty-state illustration, and a reworked device list.
  - **Dark mode**: the expand/collapse chip, the toggles and the translation progress bar were rendering in the brand primary, which has no contrast against the dark surfaces (`.dark-theme` never redefines `--primary-color`). All three now use neutral greys, so a single rule serves both themes.
  - Missing `menu.activity`, `menu.connected-devices` and `menu.about` strings added to `es`/`ca` — they only existed in `en`, so those entries rendered their raw key.
