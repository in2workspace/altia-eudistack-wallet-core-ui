...
- **Wallet sessions could be silently closed with no warning, sometimes less than 20 minutes after login (E-02)**: when the automatic background token refresh failed, `RemoteAuthService` redirected straight to the login screen with no explanation. It now shows the existing "your session has expired" notice in that case too. A redundant duplicate `forceLogout()` call on the same failure was also removed.
  - Some error toasts (`errors.default`, `pin-expired`, `incorrect-pin`, `failed-qr-process`, `loading-VCs`, `navigation`) render a "contact support" link that used to show up as raw, unrendered HTML tags instead of a clickable link, because the message escaping added for EUD-142 (F2) — kept in place — was also escaping that intentional markup. The support link is now built in code from a placeholder instead of being embedded in the translated string.
  - A second, unrelated "something went wrong" toast could stack on top of the dedicated session-expired one when another in-flight request failed right after `forceLogout()`. `HttpErrorInterceptor` now skips its generic toast once the user is already logged out.
